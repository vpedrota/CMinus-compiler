...
def gerar_quadruplas(saida, df):
    ...
    for pos_quad, quad in enumerate(quads):
        ...
        if  quad[0] == "FUN":
            ...
            resultado = df.query("Nome == @quad[2] and Escopo == @escopo_atual")
         
            if resultado.empty:
                escopo_nome = "global"
                resultado = df.query("Nome == @nome and Escopo == @escopo_nome")

            print(resultado)
            if resultado['Tipo_var'].values[0] == "VetorParametroInteiro":
                deslocamento = registers_quad[1]
                assembly.append("LW {} {} {}\n".format("$sp", "$t25", resultado['memory_position'].values[0]))
                assembly.append("ADD {} $t{} $t{}\n".format("$t25", registers_quad[1], deslocamento))
                assembly.append("SW $t{} $t{} {}\n".format(registers_quad[1], registers_quad[0], 0))
            else:
            ...
            escopo_novo = quad[2]
            nome_var = quad[1]
            registrador = registers_quad[0]
            print(escopo_atual)
            resultado = df.query("Nome == @nome_var and Escopo == @escopo_atual")
            if resultado.empty:
                resultado = df.query("Nome == @nome_var and Escopo == 'global'")
            
            if(resultado['Tipo'].values[0] ==  "Vetor"):
                mem_pos = buscar_dados(df, nome_var, escopo_atual)
                if mem_pos == "-1":
                    mem_pos = buscar_dados(df, nome_var, "global")
                    assembly.append("ADDI {} $t{} {}\n".format("$zero", registers_quad[0], mem_pos))
                else:
                    assembly.append("ADDI {} $t{} {}\n".format("$sp",  registers_quad[0], mem_pos))
                
            elif(resultado['Tipo'].values[0] ==  "Variável" and resultado['Tipo_var'].values[0] == "inteiro"):
                mem_pos = buscar_dados(df, nome_var, escopo_atual)
                if mem_pos == "-1":
                    mem_pos = buscar_dados(df, nome_var, "global")
                    assembly.append("LW {} $t{} {}\n".format("$zero", registers_quad[0], mem_pos))
                    ...
                    assembly.append("LW {} $t{} {}\n".format("$sp",  registers_quad[0], mem_pos))
            elif resultado['Tipo'].values[0] ==  "Variável" and resultado['Tipo_var'].values[0] == "VetorParametroInteiro":
                mem_pos = buscar_dados(df, nome_var, escopo_atual)
            ...
            if resultado.empty:
                escopo_nome = "global"
                resultado = df.query("Nome == @quad[2] and Escopo == @escopo_nome")

            if resultado['Tipo_var'].values[0] == "VetorParametroInteiro":
                deslocamento = registers_quad[1]
                assembly.append("LW {} $t{} {}\n".format("$sp", registers_quad[0], resultado['memory_position'].values[0]))
                assembly.append("ADD $t{} $t{} $t{}\n".format(registers_quad[0], registers_quad[0], deslocamento))
                assembly.append("LW $t{} $t{} {}\n".format(registers_quad[0], registers_quad[0], 0))
